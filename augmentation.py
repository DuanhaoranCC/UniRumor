...
def drop_edge_weighted(data, edge_weights, p, threshold):
    ...
    return data.edge_index[:, sel_mask], data.edge_attr[sel_mask]
...
def drop_edge(batch_data, aggr, p, threshold):
    ...
    for i in range(aug_data.num_graphs):
        if aug_data_list[i].num_nodes > 1:
            ...
            aug_edge_index, aug_edge_attr = drop_edge_weighted(aug_data_list[i], edge_weights, p, threshold)
            aug_data_list[i].edge_index = aug_edge_index
            aug_data_list[i].edge_attr = aug_edge_attr
...
def drop_node(batch_data, p, threshold):
    ...
    for i in range(aug_data.num_graphs):
        ...
        sel_mask[0] = True
        aug_edge_index, aug_edge_attr = tg_utils.subgraph(sel_mask, aug_data_list[i].edge_index, aug_data_list[i].edge_attr, relabel_nodes=True,
